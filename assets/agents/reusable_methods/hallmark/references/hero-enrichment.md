...
- Frame treatment (hairline 1 px frame · none)

**Example.** A SaaS observability page can place a hand-built CSS-art trace waterfall beside the display headline, tilted -0.4° and extending 12 vw past the viewport's right edge. Use a hairline frame. **Not a real video** — the mockup is custom-built CSS at Tier A (rectangles on a percentage grid simulating a flame chart). Mobile (< 60 rem): drop the clip, stack vertically.
...
**Example.** A small fashion brand's spring lookbook can use an 8-second muted loop of fabric draping in a studio. Apply `mix-blend-mode: multiply` over a 0.5-opacity warm-cream overlay so the display headline remains readable. Pause on hover so the user can read the lede without distraction. Caption track (VTT) describes the footage for accessibility.

### E3 · Mock App Screenshot — Minimal-frame split

Display headline left, a real app screenshot right, slightly tilted (1–3°) when the composition benefits. Use at most a minimal hairline border; do not draw browser chrome or macOS dots.
...
**Knobs:**
- Frame style (minimal hairline · none)
- Tilt angle (0° · 1.5° · 3°)
- Screenshot count (1 · stack-of-3 · orbit-of-3)

**Example.** A project-tracker landing page can place its real kanban screenshot beside the headline, tilted 1.5° clockwise with a minimal hairline border. Three numbered annotations can use small numbered pins and margin-aligned captions—never arrows-and-labels. Prefer one screenshot over a stack: fewer assets to load and a sharper reading path.
...
*Avoid when:* the screenshot is too weak to stand on its own; improve the capture rather than adding decorative chrome.
...
**Example.** A bakery page can place letter-style hero copy beside a 60-line hand-built SVG loaf with body, shade, and score-mark paths. Animate it with `@property --rise` for a subtle 4 px breathing loop over 6 s; use a static reduced-motion fallback.
