...
Loaded eagerly by SKILL.md Step 3 whenever the catalog pick is `cobalt`. The OKLCH palette + font stack live in [`tokens.css`](../tokens.css) under `[data-theme="cobalt"]`. This file supplies the API-product landing guidance without depending on an external example payload.
...
Plus the Space Grotesk + Inter + JetBrains Mono link and the small reveal/type-in script. Adapt the pattern to the project rather than mirroring an external example.
