 ...
 * axes: <paper-band> / <display-style> / <accent-hue> · manual-review: recorded · studied: no
 */
```
...
2. **Does not save themes for reuse.** A custom run is per-output. The skill does not write back to [`tokens.css`](tokens.css). If the user wants a permanent theme, they explicitly authorize the project change.
