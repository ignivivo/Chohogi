...
- UI 탐색, 구성, 문서, 레거시 변경은 수용 조건과 직접 검증에 맞는 방법을 고른다. 실질적인 사용자 대면 UI 추가·재설계·시각 상호작용 변경에서는 먼저 `frontend-surface`로 과업·위계·구성·기존 시스템 적합성·후속 위험 handoff를 검토한다. 새 표면 설계·감사·시각 redesign·사용자 제공 참조의 비복제 study가 필요하면 그 뒤에 `hallmark`를 적용한다. 두 방법은 UI 내부의 검토 순서일 뿐 route·권한·완료 조건을 선택하는 controller가 아니다.
